...
- We then move on to Model building which will be a simple keras classifier model that has been converted to an object detector. Essentially it is a convolution neural network(in this case base skeleton of MobileNet)which pre-trained  on a ImageNet DS who’s top layer gets replaced by two heads , namely a regression head 
